...
Problem statement: Tufts students often have trouble deciding whether to eat at Carmichael or Dewick-Macphie dining hall. Dining Duel makes the choice easy, with a compiled cumulative score for each dining hall.

Solve the problem: We make it easy to decide on the right place to go, by compiling a cumulative score for each dining hall at a specific meal.  We will use a database and server-side score-compilation algorithms to create scores between Tufts’ two dining halls. 
...
Data: Our prototype will be collecting the following data, stored in a MongoDB database:
...
Algorithms or special techniques:
    ...
    * Our algorithm for our cumulative score is the following, where the sums are over the food items upvotesupvotes + downvotes * itemweight itemweight
